...
impl Default for PathCell {
    fn default() -> Self {
        Self { up: [u32::MAX; 3], down: [u32::MAX; 3], right: [u32::MAX; 3], left: [u32::MAX; 3] }
    }
}
...
impl PathCell {
    ...
    pub fn min_costs(&self) -> u32 {
        *self.right.iter().chain(self.left.iter()).chain(self.up.iter()).chain(self.down.iter()).min().unwrap()
    }
}
